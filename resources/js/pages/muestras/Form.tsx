...
export const Form = ({ id, cuentas, locals, onClose, processing, onStore, onGetItem, onReload }: any) => {
	...
	useEffect(() => {
		if (id) {
			axios.get(route('api.muestras.show', { muestra: id })).then((res) => {
				const item = res.data;
				setData({
	...
	const toggleTag = (tag: string) => {
		...
		if (current.includes(tag)) {
			setData('etiquetas', current.filter((t) => t !== tag));
		} else {
	...
	const submit: FormEventHandler = async (e) => {
		...
		try {
			await onStore(
				() => ({ url: route('api.muestras.store') }),
				() => ({ url: route('api.muestras.update', { muestra: id }) }),
				data,
				!!id, // isUpdate
				(err: any) => {
				...
				<form onSubmit={submit}>
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-slate-900">
						...
						{isSuperAdmin && (
							<SelectField
								...
								onChange={(val) => {
									setData((old) => ({ ...old, cuenta_id: val as string, referencia_id: '', inventario_id: '', variante: '', etiquetas: [] }));
									setSelectedRef(null);
									setSelectedInv(null);
								}}
								lista={cuentas}
					...
					{selectedInv && selectedRef?.categoria?.subdivision_stock && (
						<div className="mt-6 space-y-3">
							...
							<div className="grid grid-cols-2 sm:grid-cols-3 gap-4 p-4 bg-slate-50 rounded-lg border border-slate-200">
								{selectedRef.categoria.subdivision_stock.map((tag: string) => (
									<div key={tag} className="flex items-center space-x-2">
										<Checkbox
											id={`tag-${tag}`}
											checked={data.etiquetas.includes(tag)}
											onCheckedChange={() => toggleTag(tag)}
										/>
										<Label htmlFor={`tag-${tag}`} className="text-sm cursor-pointer unselectable flex items-center gap-2">
											{tag}
											{selectedInv.subdivision_stock?.[tag] > 0 && (
												<Badge variant="outline" className="text-[10px] bg-green-50 text-green-700 border-green-200">Disponible suelto</Badge>
											)}
										</Label>
									</div>
								))}
