...
export const TransferModal = ({ isOpen, onClose, cuentas, referenciasInit }: any) => {
	...
	const fetchInventory = async (refId: string) => {
		...
		setLoading(true);
		try {
			const res = await axios.get(route('api.traslados.inventory', {
				referencia_id: refId,
	...
	const selectedOriginItem = useMemo(() => {
		return inventoryDetails.find(item =>
			String(item.estanteria_id) === String(originEstanteria) &&
			String(item.talla) === String(originTalla)
		);
	...
	const handleApplyTransfer = () => {
		...
		setProcessing(true);
		router.post(route('traslados.store'), {
	...
	return (
		<Modal
			...
			onClose={onClose}
			closeable={true}
			title="Realizar Traslado de Mercancía"
			maxWidth="2xl"
		>
						...
						<div className="space-y-3">
							...
							{loading && (
								<div className="flex items-center gap-2 text-xs text-slate-400 p-2 italic">
									<Loader2 className="animate-spin h-3 w-3" /> Buscando stock disponible...
								</div>
							)}
						</div>
					</div>
						...
						<div className="bg-slate-100 rounded-lg p-4 flex items-center justify-between border border-slate-200">
							<div>
								...
								<div className="flex items-center gap-2">
									<Badge variant="secondary" className="font-mono">Talla {selectedOriginItem.talla}</Badge>
									<span className="text-sm font-medium">desde <span className="font-bold">{selectedOriginItem.bodega_nombre} ({selectedOriginItem.estanteria_nombre})</span></span>
								</div>
							</div>
							<Button variant="ghost" size="sm" onClick={() => setStep(2)} className="text-xs" disabled={processing}>← Cambiar origen</Button>
						</div>
						...
						<div className="flex flex-col items-center py-4 space-y-4">
							<div className="w-full max-w-[200px]">
								<InputField
									...
									disabled={processing}
								/>
								<p className="text-[10px] text-center text-slate-400 mt-1 uppercase">Máximo disponible: {selectedOriginItem.stock}</p>
							</div>

							<Button
								size="lg"
								className="w-full h-14 text-md font-bold gap-2 shadow-xl"
							...
							>
								{processing ? <Loader2 className="animate-spin h-5 w-5" /> : <ArrowLeftRight className="h-5 w-5" />}
