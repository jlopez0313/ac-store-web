...
export const AccesoModal = ({ isOpen, onClose, bodega, local, onSuccess }: any) => {
    ...
    const handleSave = async () => {
        setProcessing(true);
        try {
            await axios.post(route('bodegas.accesos.update', { bodega: bodega.id, user_id: local.id }), {
                can_view: canView,
                can_order: canOrder,
                descuento: descuento,
            });
        ...
        >
            <div className="space-y-6 p-6">
                <div className="space-y-4">
                    <div
                        className="border-border hover:bg-muted flex cursor-pointer items-start space-x-3 rounded-lg border p-4 transition-colors"
                        onClick={() => setCanView(!canView)}
                    >
                        <Checkbox id="can_view" checked={canView} onCheckedChange={(v) => setCanView(v as boolean)} />
                        <div className="text-sm select-none">
                            <p className="text-foreground font-bold">Ver Inventario</p>
                            <p className="text-muted-foreground text-xs">Permite que este usuario consulte el stock actual de la bodega.</p>
                        </div>
                    </div>
                    ...
                    >
                        <Checkbox id="can_order" checked={canOrder} onCheckedChange={(v) => setCanOrder(v as boolean)} />
                        <div className="text-sm select-none">
                            <p className="text-foreground font-bold">Pedir Stock</p>
                            <p className="text-muted-foreground text-xs">
                                Permite que este usuario genere solicitudes de traslado desde esta bodega.
                            </p>
                        </div>
                    </div>
