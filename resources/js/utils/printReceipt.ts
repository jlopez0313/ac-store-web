...
function buildTicketHtml(item: PrintItem, facturaId: number, localName: string, footer: string): string {
    ...
    return `
        <div class="ticket">
            <div class="row">
                <span><b>REF</b> ${item.producto.codigo}</span>
                <span><b>EST</b> ${item.estanteria_nombre}</span>
                <span><b>FAC</b> ${facturaId}</span>
            </div>
...
export function printReceipts(data: PrintData): void {
    const footer = data.footer || 'TENISOFT / WhatsApp / 304 522 292';
